...
class KBest(SortedCollection):
    
    def __init__(self,maxCapacity,key,getter):
        SortedCollection.__init__(self,[],key)
        self.maxCapacity = maxCapacity
        self.getter = getter
        
    def insert(self,new):
        SortedCollection.insert(self,new)
        if len(self) > self.maxCapacity:
            self.pop()
            
    def max(self):
        if len(self.items()) > 0:
            return self.getter(self.items()[0])
        else:
            return 0
